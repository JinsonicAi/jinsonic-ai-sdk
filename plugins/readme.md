# View plug-in configuration information

```shell
apt install -y libarchive-tools jq
bsdtar -xOf osd_plugin.plugin config.json | jq
LC_ALL=C bsdtar -xOf plugins_out/hdmi_plugin.plugin config.json | jq '.params[].name'
```
