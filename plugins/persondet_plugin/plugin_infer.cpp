...
extern "C" void plugin_init(SDKInterface* sdk) {
	...
	sdk->register_node(PLUGIN_NODE_NAME, [](const std::string& name, const nlohmann::json& config) {
		...
		nodeParams->cap_interval		= jp(config, "cap_interval", 10);
		...
		nodeParams->region_rule.loiter_sec        = jp(config, "region_loiter_sec", 10);
		nodeParams->region_rule.loiter_cooldown   = jp(config, "region_loiter_cooldown", 30);
		nodeParams->region_rule.enable_absence    = jp(config, "region_enable_absence", false);
		nodeParams->region_rule.absence_sec       = jp(config, "region_absence_sec", 30);
		nodeParams->region_rule.absence_cooldown  = jp(config, "region_absence_cooldown", 60);
		nodeParams->region_rule.enable_crowd      = jp(config, "region_enable_crowd", false);
		nodeParams->region_rule.crowd_threshold   = jp(config, "region_crowd_threshold", 10);
		nodeParams->region_rule.crowd_recover_gap = jp(config, "region_crowd_recover_gap", 2);
		nodeParams->region_rule.enable_line_cross = jp(config, "region_enable_line_cross", false);
		nodeParams->region_rule.label_filter      = jp(config, "region_label_filter", -1);
		// Default min bbox size comes from mini_human unless user overrides.
		nodeParams->region_rule.min_bbox_size    = jp(config, "region_min_bbox_size", nodeParams->mini_human);
		...
		nodeParams->llm_review.timeout_ms			 = std::max(200, jp(config, "llm_review_timeout_ms", 1200));
