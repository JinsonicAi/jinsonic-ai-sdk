...
public:
	...
	// Additive overload: the historical five-argument constructor and symbol stay
	// unchanged. Reusable memory is an allocation hint; exhaustion safely falls
	// back to the default allocator.
	AXVideoFrame(uint32_t width, uint32_t height, int device_id,
				 AX_IMG_FORMAT_E format, AX_U32 nAlign, MemoryPolicy policy);
	AXVideoFrame(uint32_t width, uint32_t height, int device_id, AX_U32 size);
	// Exact-size reusable buffer. This preserves the source frame byte layout
	// (including non-default decoder strides) while allowing hot-path clones to
	// reuse local AX_POOL blocks. Existing constructor symbols and object layout
	// remain unchanged.
	AXVideoFrame(uint32_t width, uint32_t height, int device_id, AX_U32 size,
				 MemoryPolicy policy);
	...
	// Copy a complete frame into this already allocated frame.  Matching local
	// AX frames use the VPP synchronous engine first; unsupported domains or a
	// busy/failed VPP operation fall back to the validated DMA/CPU path.
	int CopyFrameFrom(const AXVideoFrame& source);
	// src_device_id:
