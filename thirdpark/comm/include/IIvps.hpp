...
public:
	...
	// General perspective/affine transform: accepts an externally computed 3x3 inverse-mapping
	// matrix (row-major, dst->src, floating-point). Internally converts to fixed-point (×1e6)
	// and feeds it to GDC, bypassing CalculateTransformMatrices.
	// For affine transforms, matrix[6..8] should be {0, 0, 1}.
	virtual std::shared_ptr<AXVideoFrame> HwIvpsDewarpMatrix(const AX_VIDEO_FRAME_T* ptSrcFrame, std::pair<int, int> Size, const double (&matrix)[9],
															 const ResizeOptions& options = ResizeOptions{}) = 0;
	virtual std::shared_ptr<AXVideoFrame> HwIvpsCropResize(AX_VIDEO_FRAME_T* ptSrcFrame, std::pair<int, int> Size, AX_IVPS_RECT_T crop, IVPS_ENGINE_ID_E eEngineId = IVPS_ENGINE_ID_VPP,
														   const ResizeOptions& options = ResizeOptions{})	 = 0;
};
#endif
