...
class AffineMatrix {
public:
	cv::Mat init(const cv::Mat& image, const cv::Size& size, const ResizeOptions& options = ResizeOptions{}, bool percentage = false) {
		if (percentage) {
		...
		if (options.mode == ResizeMode::Stretch) {
			i2d = (cv::Mat_<float>(2, 3) << scale_x, 0, 0,
				   0, scale_y, 0);
		} else {
			if (options.mode == ResizeMode::LetterboxCenter) {
				tx = (-scale * image.cols + size.width + scale - 1) * 0.5f;
				ty = (-scale * image.rows + size.height + scale - 1) * 0.5f;
			}
			i2d = (cv::Mat_<float>(2, 3) << scale, 0, tx,
				   0, scale, ty);
		}
		...
		cv::Mat input_image;
		const cv::Scalar bgColor(options.bgColor & 0xFF,
							  (options.bgColor >> 8) & 0xFF,
							  (options.bgColor >> 16) & 0xFF);
		cv::warpAffine(image, input_image, i2d, size,
					   cv::INTER_NEAREST, cv::BORDER_CONSTANT, bgColor);

		warpAffineFlag_ = 1;
		return input_image;
	}

	cv::Mat init(const cv::Mat& image, const cv::Size& size, bool percentage) {
		return init(image, size, ResizeOptions{}, percentage);
	}
	...
	T inv(const T& obj) const {
		...
		if constexpr (has_rect<T>::value) {
			...
			originalObj.rect.y		= (d2i_[4] * obj.rect.y + d2i_[5]) / org_height;
			originalObj.rect.width	= d2i_[0] * obj.rect.width / org_width;
			originalObj.rect.height = d2i_[4] * obj.rect.height / org_height;
		}
	...
	transformKeypoints(T& obj, const std::vector<float>& d2i_) const {
		if constexpr (has_landmarks<T>::value) {
			for (auto& point : obj.landmarks) {
				...
				float original_y = d2i_[4] * point.y + d2i_[5];
