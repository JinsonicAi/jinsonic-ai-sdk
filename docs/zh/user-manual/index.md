...
!!! tip "AI 助手图文指南"
    通过对话创建和管理视频任务、查询报警、日志与关联录像，请参阅 [AI 助手使用说明](../ai-assistant/index.md)，含分步骤截图与模型服务设置。

!!! tip "通过云平台远程管理设备"
    从设备端云端配置、一次性绑定到远程任务、实时监控和报警处置，请阅读 [《云端管理平台使用说明》](../cloud-platform/index.md)。

> 版本：1.2 | 文档类型：用户操作指南 | 面向对象：终端用户、运维人员

!!! tip "首次开箱？"
    如果你刚拿到设备、还没连上 Web 平台，请先阅读 [《设备开箱与快速上手》](../device-onboarding.md)，完成物理连接、获取 IP、安装 SDK 后再回到本手册了解各项功能的详细用法。

**按使用角色快速定位**

- **初次使用**：[登录](#三快速入门登录) → [任务管理](#五任务管理) → [流程编辑器](#六流程编辑器详解) → [综合实例](#十九综合实例演示)
- **日常使用**：[视频预览](#十一视频预览与多路预览) · [报警管理](#十二报警管理) · [人脸库与录像](#十四插件功能人脸库与录像管理)
- **系统运维**：[插件管理](#十三插件管理) · [系统管理](#十七设置与系统管理) · [常见问题与排障](#二十一常见问题与排障)
...
从登录到看到报警，平台的核心使用流程如下：

```mermaid
flowchart LR
    A[登录平台] --> B[创建任务]
    B --> C[拖拽节点<br/>输入·算法·输出]
    C --> D[连接数据流<br/>配置参数]
    D --> E[保存并运行]
    E --> F[视频预览]
    E --> G[查看报警]
...
> 输入源（RTSP 摄像头等）→ 算法（行人检测、人脸识别等）→ 输出（推流、报警、录像等），沿此方向连接即可构建一条完整分析流程。
...
!!! warning "连接失败时的排查"
    若连接失败，请依次检查：设备是否在线、网络是否互通、防火墙是否放行 WebRTC 相关端口。
...
1. 在浏览器地址栏输入访问地址（见 2.2）
   ...
   ![image-20260226170524729](../../assets/user-manual/imgs/image-20260226170524729.png)
...
- **成功**：自动跳转到首页（设备节点）
  ...
  ![](../../assets/user-manual/imgs/image-20260224095525553.png)

- **失败**：页面顶部或弹窗会提示错误信息（如「设备连接失败」「登录已过期」等）

!!! warning "提示「设备连接失败」时"
    请确认设备应用已启动、网络可达，然后刷新页面重试。
...
- **语言**：登录后可在主界面设置菜单中切换中文/英文
  ...
  ![image-20260226170647449](../../assets/user-manual/imgs/image-20260226170647449.png)
...
![image-20260224095943825](../../assets/user-manual/imgs/image-20260224095943825.png)
...
![image-20260224100037353](../../assets/user-manual/imgs/image-20260224100037353.png)

- **双击**可打开设备详情弹窗

  ![image-20260224100254937](../../assets/user-manual/imgs/image-20260224100254937.png)
...
- **任务卡片与分组**：见下一节
  ...
  ![](../../assets/user-manual/imgs/image-20260224172722980.png)
...
任务从创建到稳定运行会经历以下状态。遇到异常时，先判断问题发生在“配置”“运行”还是“输出验证”阶段，再进入对应章节排查。
...
3. 弹出创建对话框，输入名称后创建
   ...
   ![](../../assets/user-manual/imgs/image-20260224172759383.png)

![image-20260224100723620](../../assets/user-manual/imgs/image-20260224100723620.png)
...
2. 打开流程编辑弹窗，可修改节点与连线（见第六章）
   ...
   ![](../../assets/user-manual/imgs/image-20260224172830271.png)
...
!!! warning "设备未连接时"
    设备未连接时无法启动或停止任务，请先恢复设备连接。

![](../../assets/user-manual/imgs/image-20260224172855955.png)
...
3. 系统会自动创建一个副本，名称为「原名称 + 拷贝」
   ...
   ![](../../assets/user-manual/imgs/image-20260224172911487.png)
...
- 任务卡片**右键** → 「移出分组」（若任务在分组内）
  ...
  ![](../../assets/user-manual/imgs/image-20260224172939435.png)

![](../../assets/user-manual/imgs/image-20260224173002950.png)
...
- 点击可打开报警面板并定位到相关报警
  ...
  ![](../../assets/user-manual/imgs/image-20260224173437963.png)
...
- 展开状态下，右下角有**预览**、**编辑**悬浮按钮；悬停时还会显示**启动/停止**、**更多**按钮
  ...
  ![](../../assets/user-manual/imgs/image-20260224173513448.png)
...
- 例如：一个任务只能有一个 RTSP 输入（除非插件支持多路），需删除原有或选择其他输入类型
  ...
  ![image-20260224102653931](../../assets/user-manual/imgs/image-20260224102653931.png)
...
!!! warning "连接方向"
    不可从输出节点连到输入节点，也不可反向连接；必须沿 **输入 → 算法 → 处理 → 输出** 的方向。

![image-20260224102907296](../../assets/user-manual/imgs/image-20260224102907296.png)
...
3. 需点击编辑器顶部「保存」或「运行」才会同步到设备
   ...
   ![image-20260224103021626](../../assets/user-manual/imgs/image-20260224103021626.png)

!!! danger "配置不会自动保存"
    在弹窗中点「确定」只是写入当前节点，**不会同步到设备**。务必点击编辑器顶部的「保存」或「运行」，否则修改将在刷新或退出后丢失。
...
- **动态**：由设备自动分配；指定位置则固定到该计算单元
  ...
  ![image-20260224103208147](../../assets/user-manual/imgs/image-20260224103208147.png)
...
7. 保存后，任务会在设定时段内自动运行，非时段内自动暂停
   ...
   ![image-20260224103400422](../../assets/user-manual/imgs/image-20260224103400422.png)
...
- 「撤销」「清除全部」可管理历史；支持多区域，保存后生效
   ...
   ![image-20260224103806790](../../assets/user-manual/imgs/image-20260224103806790.png)
...
![image-20260224104154250](../../assets/user-manual/imgs/image-20260224104154250.png)
...
```mermaid
flowchart LR
   I[输入节点<br/>RTSP / 网络拉流] --> A[算法节点<br/>检测 / 识别 / LLM]
   A --> P[处理节点<br/>OSD / 属性分析]
   P --> V[视频输出<br/>网络推流 / HDMI / 录像]
   A --> E[事件输出<br/>报警 / 平台对接]
```

!!! info "先确认输出目标"
   - 要在网页看画面：流程中必须有**网络输出**节点。
   - 要收到报警：流程中必须有**报警输出**节点。
   - 要保留录像：流程中必须有**录像输出**节点，并确认设备存储空间充足。
...
!!! warning "预览的两个前提"
   1. 任务流程中必须包含**网络输出**节点。
   2. 任务必须处于**运行中**状态，否则会提示「请先启动任务后再预览」。

  ![](../../assets/user-manual/imgs/image-20260224104254841.png)

  ![image-20260224104316417](../../assets/user-manual/imgs/image-20260224104316417.png)
...
- 可查看「行人检测」「火焰检测」「烟雾检测」等分类统计
  ...
  ![image-20260224104405893](../../assets/user-manual/imgs/image-20260224104405893.png)
...
!!! danger "生产环境变更原则"
   插件替换、禁用、卸载和回滚都可能影响正在运行的任务。操作前应确认引用关系、停止相关任务，并安排业务低峰窗口；操作后必须核对插件状态和任务运行情况。
...
!!! danger "不要忽略兼容性警告"
   测试环境验证通过不等于可以直接用于生产。部署前应核对设备架构、系统版本、芯片平台和插件依赖，并保留可回滚版本。
...
7. 页面重新连回设备后，确认插件状态是否变为 **已加载**。
   ...
   ![image-20260325191452247](../../assets/user-manual/imgs/image-20260325191452247.png)
...
- 页面在“上传完成后仍继续等待”的情况，通常表示系统仍在处理安装，请先耐心等待。
  ...
  ![image-20260325191600317](../../assets/user-manual/imgs/image-20260325191600317.png)

#### 13.2.3 安装前建议核对的信息

- [ ] 插件是合法的 `.plugin` 文件。
- [ ] 插件类型与计划用途一致，例如 `llm.plugin`、`record.plugin`、`persondet.plugin`。
- [ ] 插件版本符合本次发布计划。
- [ ] 当前设备架构、系统版本、芯片平台与插件兼容。
- [ ] 设备磁盘空间充足，避免上传完成后因空间不足导致失败。
- [ ] 已确认受影响任务和可用的回滚版本。
...
!!! warning "回滚不是自动兜底"
   只有系统保留了上一版本备份包时才能回滚。升级前请先确认回滚入口和历史版本可用，不能把“可能存在备份”当作恢复方案。
...
![image-20260324111801159](../../assets/user-manual/imgs/image-20260324111801159.png)
...
!!! warning "人脸数据安全"
   人脸照片和身份字段属于敏感数据。导入、导出和备份文件应按项目的数据安全要求保存和传输，避免通过公共链接或非受控设备流转。
...
- 建议提前规划人脸库分组、命名规范、`PID/工号` 等唯一标识，降低重复数据和人工维护成本
   ...
   ![image-20260224104746448](../../assets/user-manual/imgs/image-20260224104746448.png)
...
![](../../assets/user-manual/imgs/image-20260224173702412.png)
...
适合已有检测算法并希望降低误报的场景。链路为 `RTSP → 算法节点 → LLM → 报警`，原算法先筛选候选目标，LLM 再做二次判断。

#### 直接检测

适合按自然语言描述直接判断画面的场景。链路为 `RTSP → LLM → 报警`，需要重点调整提示词和检测间隔。

!!! tip "模式选择"
   已有成熟算法时优先使用复核模式；只有在现有算法无法覆盖目标语义时，再考虑检测模式。最终效果应以现场画面、提示词和实际报警结果验证为准。
...
![image-20260228153334897](../../assets/user-manual/imgs/image-20260228153334897.png)

![image-20260228153528927](../../assets/user-manual/imgs/image-20260228153528927.png)
...
![image-20260228153644781](../../assets/user-manual/imgs/image-20260228153644781.png)

![image-20260228153803764](../../assets/user-manual/imgs/image-20260228153803764.png)
...
本章介绍设备页面顶部放大镜打开的独立智能检索窗口。若希望直接在会话中说出画面描述并查看结果，请参阅 [AI 助手：通过会话进行智能索引搜索](../ai-assistant/index.md#intelligent-retrieval)。
...
- [ ] 已有需要查找的报警图片或保存录像，且文件仍可访问。
- [ ] 在顶部齿轮的 **智能检索设置** 中打开 **启用检索**、选择运行位置并保存。
- [ ] 检索服务已就绪，历史媒体已建立索引；已有数据但索引为空时，可点击 **立即同步** 后等待完成。
- [ ] 已确认检索范围是图片、视频或全部。

查询已有索引不要求原任务此刻仍在运行。助手的 LLM 模型服务与智能检索分别设置，打开助手不等于开启检索。

### 16.3 操作步骤

#### 16.3.1 文字检索

1. 点击设备页面顶部的**放大镜“智能检索”**，打开独立检索窗口。
2. 输入搜索内容。
3. 选择检索范围。
4. 点击搜索并查看结果。
...
| 现象 | 处理方式 |
|------|----------|
| 搜索无结果 | 区分检索未启用、服务未就绪、空索引和没有匹配；确认媒体仍保留且已完成索引，必要时在检索设置中立即同步 |
...
!!! danger "升级前必须确认"
   - 使用与设备型号匹配的安装包或固件，升级前记录当前应用和固件版本。
   - 升级会中断当前业务，安排维护窗口并停止关键任务。
   - 上传、安装和设备重启期间不要断电、刷新页面或重复提交。
   - 升级后重新登录，检查设备版本、插件状态和关键任务。
...
- 包含：设备名称、应用版本、固件版本、序列号、系统信息等
  ...
  ![image-20260224104914966](../../assets/user-manual/imgs/image-20260224104914966.png)
...
!!! success "任务验收清单"
   - [ ] 任务状态为“运行中”，没有运行警告或异常。
   - [ ] 视频画面连续，延时和帧率符合现场要求。
   - [ ] 检测框位置正确，目标进入检测区域时能够稳定识别。
   - [ ] 报警能够产生，抓图、时间和任务名称正确。
   - [ ] 页面刷新或设备服务重启后，任务配置仍然存在且可恢复运行。
...
![image-20260224105012045](../../assets/user-manual/imgs/image-20260224105012045.png)
...
!!! info "清单不是安装承诺"
   固件版本、项目裁剪和授权范围都可能影响实际插件集合。实施和验收时应以设备“插件管理”页面中显示的名称、版本和状态为准。
...
flowchart TD
   ...
   B -- 否 --> C[检查设备应用、网络和防火墙]
   B -- 是 --> D{任务能否启动?}
   D -- 否 --> E[检查节点配置、插件状态和保存结果]
   D -- 是 --> F{有视频输出?}
   F -- 否 --> G[检查网络输出节点和任务运行状态]
   F -- 是 --> H{有预期报警?}
   H -- 否 --> I[检查算法阈值、检测区域和报警输出节点]
   H -- 是 --> J[业务链路正常]
```

!!! tip "排障顺序"
   按“设备连接 → 插件状态 → 任务配置 → 节点运行状态 → 输出结果”的顺序排查。先解决上游问题，再检查视频、报警和录像等下游现象，避免同时反复修改多个参数。
