...
AI 助手是 AI-BOX Web 页面中的视频任务工作入口。你可以通过对话咨询检测方案、查看设备能力和[设备版本及资源指标](#device-information)、选择算法、创建检测任务、调整已有任务的运行状态，以及查询报警、日志和关联录像。助手会把需求转成具体的业务操作，并在需要时提供算法选择、摄像头地址、目标任务和确认表单。
