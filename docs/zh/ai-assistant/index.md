...
AI 助手是 AI-BOX Web 页面中的视频任务工作入口。你可以通过对话咨询检测方案、查看设备能力和[设备版本及资源指标](#device-information)、选择算法、创建检测任务、调整已有任务的运行状态，以及查询报警、日志、关联录像和[智能检索候选图片/视频](#intelligent-retrieval)。助手先理解需求，再按已接入能力校验方案，并在需要时提供算法选择、摄像头地址、目标任务和确认表单。

本手册面向设备管理员、现场交付工程师和日常运维人员，按“进入助手 → 检查状态 → 创建任务 → 管理任务 → 查阅证据 → 排障”的顺序介绍使用方法。

!!! note "适用范围与图片说明"
    本页更新至 2026-09-12，新增功能以 `2.1.1-202609121751-ai-assistant-rc5` 配套实现为基线。实际可用功能取决于设备应用、网页、算法插件、独立模型扩展及硬件的组合；旧版本可能没有本文中的表单或查询入口。算法名称和运行位置以当前设备返回的列表为准。

    RC5 为候选版本，尚未通过完整商用验收，真实模型仍有理解失败的场景，详见第 16 节。文档更新不表示安装包已经可下载，也不表示设备或云平台已升级；包是否生成以实际交付清单和校验结果为准。本文原有截图为早期界面示意，未展示全部新增能力；不可用作 RC5 实机验收凭据。
...
| 文本能力 | 当前扩展真正支持纯文本推理 | 核对适配扩展和模型状态；在恢复前使用原有设备管理页面，不靠固定句式绕过语义解析 |
...
发送到会话的自然语言文本先由可用 LLM 提取意图和关键条件，包括“查看任务列表”这样的短句；模型未驻留时，该消息本身就会触发按需启动，而不是要求先创建一个 LLM 视频任务。随后设备结合已安装能力、任务状态和允许的业务接口校验。打开窗口、查看模型状态和提交已经校验的结构化表单，与一条新的自然语言消息不同，不要求额外推理。
...
RC5 在设备服务进程内保存有界的短期会话状态，区分待确认草稿、已保存任务引用和操作回执。成功保存后的“停止它”应指向原任务 ID，而不是新建副本；“其它任务”不能直接等同于当前任务或所有任务，仍需明确目标。

新的改口会使旧的未提交确认卡失效。若输出或条件核验未通过，先按提示修正，不要继续确认旧卡。当前不是完整的长期记忆：上下文成功写入后有效期为 20 分钟，服务重启后不保留，跨连接、跨浏览器的自动续聊尚未实现；容量或内容超限会明确拒绝。重连后重新查询实际任务并核对 ID，聊天历史可见不代表服务端仍记得所有条件。复杂的连续撤回、替换条件仍需检查新方案，不能假定任意改口都已正确合并。
...
RC5 增加了两类校验，但不等于已支持任意组合：

- **同一目标属性条件**：例如“发现同一个人同时满足男性、戴帽时报警”，必须由行人属性组件的条件适配器生成对应规则和“全部满足”的关系，不能只勾选一个算法后使用默认“未戴帽”规则。当前生产适配器以行人属性为主；颜色、跨目标关系、先后顺序、持续时间或其它组件未支持的条件，不能静默丢掉后创建。属性类别是算法预测，不是身份或真实性确认。
- **明确输出要求**：新建或采用建议时，明确要求录像、HDMI 或输出限制会单独提取、核对；已有待确认草稿走草稿变更校验，不重复处理同一变更。普通肯定报警、网络预览属于默认输出，不额外重复节点。要求关闭默认报警/网络输出，或“仅保留 HDMI”等当前模板不能保证的限制，会提示暂不支持，不能当作已执行。遇到提取失败，修正需求并检查下一张完整方案；仅换运行位置不能视为同意丢弃原输出要求。

“通知我”在这里表示准备相应检测与报警方案，不是已创建跨会话通知订阅，更不是通知已经送达。必须继续完成摄像头、确认、实际任务启动以及报警目的地配置，并用命中/不命中样本和真实通知回执验收。
...
### 8.3 用画面描述进行智能检索 {#intelligent-retrieval}
...
这是查找已有索引内容，不是创建检测任务。助手读取智能检索的真实状态，再调用已有文字检索接口，在会话中展示最多 20 个候选项。点击候选图片或视频再加载媒体；视频按返回的片内偏移定位，不把偏移伪装成事件日期。

| 返回状态 | 你需要做什么 |
|---|---|
| 智能检索尚未启用 | 在设备菜单的 **智能检索设置** 中使能并保存，等待服务和索引就绪后重试 |
| 服务未启动、正在预热或配置切换中 | 按提示检查模型、配置和资源，等待就绪；这不是“没有结果” |
| 索引为空或正在构建 | 确认已有需要索引的报警图片或录像，等待索引完成 |
| 可搜索但没有匹配 | 调整画面描述；不能因此认定现场从未出现该目标 |
| 媒体加载失败或断线 | 恢复连接后使用媒体重试入口，检查文件是否仍保留 |

会话查询不会擅自开启智能检索或修改录像设置；说“我已经开启了”也不能代替设备状态检查。范围仅包括已索引的报警图片和保存录像的抽帧，不是所有摄像头完整历史。相似度是排序分数，不是性别、颜色或身份确认的概率，需要人工核对候选画面。

当前没有稳定分页、全库精确计数或跨摄像头身份关联。任意日期、指定摄像头/通道等当前检索接口不能保证的筛选条件会提示不支持，不能悄悄改成全库搜索。需要取消这些限制时，明确发起不带该限制的新查询；“只看视频”等追问仍要核对保留的描述和筛选。智能检索与第 8.1 节报警统计是不同入口，不能把报警统计的时间范围能力套用到画面检索。
...
“按需”指没有模型消费者时不必常驻，不是跳过聊天语义理解。发送一条自然语言消息就需要 LLM 解析；未启动时自动请求启动，已加载时复用。包括简短的任务、算法和设备信息查询，都不能依靠未经过模型理解的关键词命中伪装成智能响应。
...
助手与视频 LLM 节点共享一个模型服务。普通打开窗口、读取服务状态、保存运行策略不会主动加载模型；已校验表单的确定/取消和结果重查也不等同于再次解析自然语言。模型首次加载可能明显慢于后续请求；耗时取决于模型、存储、硬件和资源占用。没有视频任务不表示不需要模型，正在聊天本身就是使用需求；关闭助手窗口也不应中断其它视频 LLM 使用者。
...
| 发送消息后一直“尚未启动” | 本次请求是否启动/复用模型，前后端是否配套 | 刷新服务状态并查看错误；RC5 聊天应先解析，不能以旧版直达查询说明为依据 |
| 智能检索提示未启用 | 设备真实检索状态 | 在智能检索设置中使能、保存，待就绪后重试 |
| 设备指标缺失或时间未知 | 设备接口是否提供字段和采样时间 | 不将缺失值当成 0；参见设备信息查询页 |
...
### 16.1 RC5 当前验证范围与已知限制

截至 2026-09-12，原生回归有 3244 项检查通过，包含 ASan/UBSan；模型和设备业务接口使用受控测试响应，不能换算成模型理解准确率。最新中英文条件专项使用真实板端模型和 ARM 规划代码，AX 为 24/26，RK 为 25/26；任务、检索等业务接口仍是隔离测试夹具，未证明真实任务落盘、摄像头检出、通知送达或长时稳定性。

此前另一源码批次的更广 66 轮业务回归为 AX 43/66、RK 37/66，仍有复合输出和多轮阶段失败，未由后续 26 轮专项完整重跑覆盖。最新专项也保留了非法属性名、无依据年龄条件、仅修改位置却进入错误阶段的反例；安全拒绝仍记为理解失败，不能算通过。两批范围和源码不同，不合并通过数。

因此 RC5 不宣称“任意聊天都精准理解”或“商用验收通过”。完整有效/未解决/已撤回条件账本、跨连接续聊、所有算法通用条件适配及真实通知闭环仍未完成；AXCL 完整硬件启动等未完成验收项也不会因升版本自动通过。
...
- [ ] 版本和资源指标与设备页面一致，单项查询/计算卡追问范围正确，缺失或过期数据有明确标识。
- [ ] 摄像头地址与画面对应正确，无凭据暴露在交付截图中。
- [ ] 自动创建与草稿创建均按实际需求完成，任务 ID 已记录。
- [ ] 先咨询再采用方案、多轮改口、输出增删和运行位置修改后，完整需求与真实任务配置一致。
- [ ] 智能检索的启用/预热/空索引/无匹配有明确区分，候选图片及视频偏移可在本地和云端打开。
- [ ] 同一目标条件与并行算法没有混淆，不能实现的条件被明确提示，实际报警及通知目的地已验证。
