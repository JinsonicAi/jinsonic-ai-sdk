...
AI 助手是 AI-BOX Web 页面中的视频任务工作入口。你可以通过对话咨询检测方案、查看设备能力和[设备版本及资源指标](device-info.md)、选择算法、创建检测任务、调整已有任务的运行状态，以及查询报警、日志和关联录像。助手会把需求转成具体的业务操作，并在需要时提供算法选择、摄像头地址、目标任务和确认表单。

本手册面向设备管理员、现场交付工程师和日常运维人员，按“进入助手 → 检查状态 → 创建任务 → 管理任务 → 查阅证据 → 排障”的顺序介绍使用方法。

!!! note "适用范围与图片说明"
    本页更新至 2026-09-11，新增功能以 `2.1.1-202609110020-ai-assistant-rc3` 配套实现为基线。实际可用功能取决于设备应用、网页、算法插件、独立模型扩展及硬件的组合；旧版本可能没有本文中的表单或查询入口。算法名称和运行位置以当前设备返回的列表为准。

    RC3 为候选版本。文档更新和安装包生成不表示所有设备或云平台已升级，也不代表所有现场场景均通过验收。本文原有截图为早期界面示意，未展示新增设备信息卡片；不可用作 RC3 实机验收凭据。
...
RC3 完整交付组合如下。先安装匹配的主包，再安装扩展；主包携带私有 Python 运行时，设备不需要自行安装系统 Python。
...
自然语言需求由可用文本模型提取意图和关键条件，再由设备结合已安装能力、任务状态和允许的业务接口校验；快捷入口和部分明确指令可直接走受控业务路径。遇到建议确认卡片，先核对建议动作再点击 **确定**。理解不可靠、条件不支持或关键信息缺失时，应按提示补充，不能把“支持自然语言”理解为任意表达都保证正确。
...
支持字段、中英文例句、缺失数据和采样时间的解释，见 [设备信息查询](device-info.md)。
...
LLM 复核是对检测结果的后续处理阶段，与“选择一个检测算法”不同。RC3 助手新建火灾烟雾任务默认加入复核，其他算法可按明确要求追加；这不会自动改写已有任务。以生成方案和节点配置为准。
...
- [设备信息查询](device-info.md)：版本、CPU/NPU、CMM、内存、温度和时效解释。
