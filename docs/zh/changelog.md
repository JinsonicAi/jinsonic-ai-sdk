...
- 新增[设备信息查询](ai-assistant/device-info.md)：软件/固件版本、设备标识、平台、时间、CPU/NPU/CMM/内存/温度，以及单项、多项和计算卡追问。
- 补充缺失值、有效零值、采样时间未知和过期数据的解释，避免把旧值说成实时值。
- 明确默认空闲释放 1800 秒、完整交付包组合、私有 Python 运行时，以及火灾默认复核和上游 Prompt/开关核对。
- 新增内容以 `2.1.1-202609110020-ai-assistant-rc3` 为基线；候选版本仍需现场验收，文档更新不等于设备或云端应用已经升级。
