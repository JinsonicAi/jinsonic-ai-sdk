...
!!! tip "First time unboxing?"
    If you just received the device and haven't connected to the web platform yet, please read [Device Onboarding & Quick Start](../device-onboarding.md) first to complete physical connection, obtain the IP address, and install the SDK, then return to this manual for detailed feature usage.

**Find content by role**

- **First-time use**: [Login](#3-quick-start-login) → [Task Management](#5-task-management) → [Flow Editor](#6-flow-editor) → [Comprehensive Example](#19-comprehensive-example)
- **Daily use**: [Video Preview](#11-video-preview--multi-view) · [Alert Management](#12-alert-management) · [Face Library & Recording](#14-plugins-face-library--recording)
- **System operations**: [Plugin Management](#13-plugin-management) · [System Management](#17-settings--system-management) · [Troubleshooting](#21-troubleshooting)
...
The core workflow from login to viewing alerts:

```mermaid
flowchart LR
    A[Log in] --> B[Create task]
    B --> C[Drag nodes<br/>input·algorithm·output]
    C --> D[Connect data flow<br/>configure params]
    D --> E[Save & run]
    E --> F[Video preview]
    E --> G[View alerts]
```

> Input source (RTSP camera, etc.) → Algorithm (pedestrian detection, face recognition, etc.) → Output (streaming, alerts, recording, etc.). Connect nodes along this direction to build a complete analysis pipeline.
...
!!! warning "Troubleshooting connection failures"
    If connection fails, check in order: device online status, network connectivity, and whether the firewall allows WebRTC-related ports.
...
1. Enter the access URL in the browser address bar (see 2.2)
   ...
   ![](../../assets/user-manual/imgs/image-20260224170235208.png)
...
- **Success**: Redirects to home (Device Nodes)
  ...
  ![](../../assets/user-manual/imgs/image-20260224170318151.png)

- **Failure**: Error message at top or in popup (e.g., "Device connection failed", "Login expired")

!!! warning "When \"Device connection failed\" appears"
    Confirm the device app is running and the network is reachable, then refresh the page and retry.
...
- **Language**: Switch Chinese/English from the main Settings menu after login.
  ...
  ![](../../assets/user-manual/imgs/image-20260224170354515.png)
...
![](../../assets/user-manual/imgs/image-20260224174021430.png)
...
- **Double-click** to open device details popup
  ...
  ![](../../assets/user-manual/imgs/image-20260224170459870.png)
  ![](../../assets/user-manual/imgs/image-20260224170529444.png)
...
- **Task cards & groups**: See next section
  ...
  ![](../../assets/user-manual/imgs/image-20260224170705764.png)
...
A task passes through the following states from creation to stable operation. When a problem occurs, first determine whether it is in configuration, execution, or output verification, then use the corresponding section to troubleshoot it.

```mermaid
stateDiagram-v2
   [*] --> Created
   Created --> Editing: add and configure nodes
   Editing --> Saved: save task
   Saved --> Running: start task
   Running --> Stopped: stop task
   Stopped --> Editing: modify configuration
   Running --> Faulted: node timeout or config error
   Faulted --> Editing: inspect and fix
   Stopped --> [*]: delete task
```
...
3. Enter name in the dialog
   ...
   ![](../../assets/user-manual/imgs/image-20260224170747775.png)
   ![](../../assets/user-manual/imgs/image-20260224170804585.png)
...
2. Opens the flow editor (see Chapter 6)
   ...
   ![](../../assets/user-manual/imgs/image-20260224170841735.png)
...
!!! warning "When the device is not connected"
    You cannot start or stop tasks while the device is disconnected. Restore the device connection first.

![](../../assets/user-manual/imgs/image-20260224170913835.png)
...
3. A copy is created with "Copy" suffix in the name
   ...
   ![](../../assets/user-manual/imgs/image-20260224170941345.png)
...
**Remove from group**: Right-click task → "Remove from Group" (when task is in a group)
  ...
  ![image-20260224171021054](../../assets/user-manual/imgs/image-20260224171021054.png)
  ![](../../assets/user-manual/imgs/image-20260224171050105.png)
...
- Click to open alert panel filtered by that task
  ...
  ![](../../assets/user-manual/imgs/image-20260224171131553.png)
...
- When expanded: **Preview**, **Edit** buttons at bottom-right; hover for **Start/Stop**, **More**
  ...
  ![](../../assets/user-manual/imgs/image-20260224171151886.png)
...
4. Release; node appears at drop location
  ...
  ![](../../assets/user-manual/imgs/image-20260224171227413.png)
...
!!! warning "Connection direction"
    You cannot connect from an output node back to an input node, nor connect in reverse. The direction must be **Input → Algorithm → Processing → Output**.

![](../../assets/user-manual/imgs/image-20260224171254513.png)
...
4. Click "OK"; config is applied locally; click "Save" or "Run" at top to sync to device
   ...
   ![](../../assets/user-manual/imgs/image-20260224171341177.png)

!!! danger "Configuration is not saved automatically"
    Clicking "OK" only applies the change to the current node — it is **not synced to the device**. Be sure to click "Save" or "Run" at the top of the editor, otherwise your changes will be lost after a refresh or exit.
...
- **Dynamic**: Device assigns automatically; specific choice fixes to that unit
  ...
  ![](../../assets/user-manual/imgs/image-20260224171440365.png)
...
5. Task runs only in enabled periods
   ...
   ![image-20260224171517103](../../assets/user-manual/imgs/image-20260224171517103.png)
...
Use Undo, Clear All as needed; supports multiple regions; save to apply.
   ...
   ![image-20260224171840049](../../assets/user-manual/imgs/image-20260224171840049.png)
...
- When schedule-paused, card shows countdown like "3d 2h 15m 30s until resume"
  ...
  ![image-20260224104002093](../../assets/user-manual/imgs/image-20260224104002093.png)
...
![](../../assets/user-manual/imgs/image-20260224171945861.png)
...
```mermaid
flowchart LR
   I[Input<br/>RTSP / network stream] --> A[Algorithm<br/>detection / recognition / LLM]
   A --> P[Processing<br/>OSD / attribute analysis]
   P --> V[Video output<br/>network / HDMI / recording]
   A --> E[Event output<br/>alert / platform integration]
```

!!! info "Decide the required output first"
   - To view video in the web UI, the flow must contain a **Network Output** node.
   - To receive alerts, the flow must contain an **Alert Output** node.
   - To retain recordings, the flow must contain a **Recording Output** node and the device must have enough storage.
...
!!! warning "Two prerequisites for preview"
   1. The task flow must contain a **Network Output** node.
   2. The task must be **Running**; otherwise the UI asks you to start it before preview.

  ![](../../assets/user-manual/imgs/image-20260224172106499.png)
  ![](../../assets/user-manual/imgs/image-20260224172126411.png)
...
- By task, by alert type
  ...
  ![](../../assets/user-manual/imgs/image-20260224172149883.png)
...
stateDiagram-v2
   [*] --> Uploaded
   Uploaded --> Installed: validate and install
   Installed --> Loaded: restart service
   Loaded --> PendingUpdate: install same-type update
   PendingUpdate --> Loaded: stop dependent tasks and restart
   Loaded --> PendingDisable: still referenced by tasks
   PendingDisable --> Disabled: stop dependent tasks
   Disabled --> Loaded: enable and restart
   Loaded --> Uninstalled: remove task dependencies first
```

!!! danger "Production change policy"
   Replacing, disabling, uninstalling, or rolling back a plugin can affect running tasks. Identify dependencies, stop related tasks, and use a maintenance window. After the change, verify both plugin status and task behavior.
...
!!! danger "Do not ignore compatibility warnings"
   Passing a test does not automatically make a package production-ready. Verify device architecture, OS version, chip platform, and runtime dependencies, and retain a known rollback version before deployment.
...
7. After the page reconnects, verify that the plugin state becomes **Loaded**.
   ...
   ![image-20260325192808496](../../assets/user-manual/imgs/image-20260325192808496.png)
...
- Even after the upload finishes, the page may still show a processing state while the device continues the installation. This is often normal.
  ...
  ![image-20260325192846394](../../assets/user-manual/imgs/image-20260325192846394.png)

#### 13.2.3 What to check before installing

- [ ] The file is a valid `.plugin` package.
- [ ] The plugin type matches the planned use, such as `llm.plugin`, `record.plugin`, or `persondet.plugin`.
- [ ] The version matches the intended release plan.
- [ ] The device architecture, OS version, and chip are compatible.
- [ ] Device storage is sufficient so installation does not fail after upload.
- [ ] Affected tasks and an available rollback version have been identified.
...
!!! warning "Rollback is not an automatic safety net"
   Rollback is available only when the system retained a previous package. Before an upgrade, confirm that the rollback action and historical version are actually available; do not treat a possible backup as the recovery plan.
...
![image-20260324111858905](../../assets/user-manual/imgs/image-20260324111858905.png)
...
!!! warning "Protect face data"
   Face images and identity fields are sensitive data. Store and transfer import, export, and backup files according to project data-security requirements. Do not distribute them through public links or unmanaged devices.
...
- For commercial projects, plan naming rules, library grouping, and unique identifiers such as `PID` or `Work ID` in advance to reduce duplicate data and manual maintenance
   ...
   ![](../../assets/user-manual/imgs/image-20260224172256374.png)
...
![](../../assets/user-manual/imgs/image-20260224172351435.png)
...
Best when an existing detector is available and false positives need a second check. The flow is `RTSP → Algorithm → LLM → Alarm`; the detector produces candidates and the LLM reviews them.

#### Direct Detection

Best for evaluating a scene directly from a natural-language description. The flow is `RTSP → LLM → Alarm`; prompt wording and detection interval require careful tuning.

!!! tip "Choosing a mode"
   Prefer Review Mode when a mature detector exists. Use Detector Mode when existing algorithms cannot express the required semantics. Validate the final result with actual site video, prompts, and alert outcomes.
...
![image-20260226160345132](../../assets/user-manual/imgs/image-20260226160345132.png)

![image-20260228153117135](../../assets/user-manual/imgs/image-20260228153117135.png)
...
![image-20260228152344262](../../assets/user-manual/imgs/image-20260228152344262.png)

![image-20260228152220259](../../assets/user-manual/imgs/image-20260228152220259.png)
...
!!! danger "Confirm before upgrading"
   - Use an application package or firmware image that matches the device model, and record the current application and firmware versions.
   - An upgrade interrupts active services. Schedule a maintenance window and stop critical tasks.
   - Do not power off the device, refresh the page, or resubmit during upload, installation, or reboot.
   - After the upgrade, log in again and verify device version, plugin status, and critical tasks.
...
- Double-click status capsule to open device details popup
  ...
  ![](../../assets/user-manual/imgs/image-20260224172426128.png)
...
!!! success "Task acceptance checklist"
   - [ ] The task is **Running** with no warning or error state.
   - [ ] Video is continuous and latency/FPS meet site requirements.
   - [ ] Detection boxes are correctly positioned and targets are detected reliably in the configured region.
   - [ ] Alerts are generated with correct snapshots, timestamps, and task names.
   - [ ] After a page refresh or service restart, the task configuration remains and operation can be restored.
...
![](../../assets/user-manual/imgs/image-20260224172512810.png)
...
!!! info "The inventory is not an installation guarantee"
   Firmware version, project customization, and licensing can change the actual plugin set. During delivery and acceptance, use the names, versions, and states shown in the device's **Plugin Manager** as the source of truth.

## 21. Troubleshooting

```mermaid
flowchart TD
   A[Page or task problem] --> B{Device connected?}
   B -- No --> C[Check device app, network, and firewall]
   B -- Yes --> D{Task starts?}
   D -- No --> E[Check node config, plugin status, and save result]
   D -- Yes --> F{Video output available?}
   F -- No --> G[Check Network Output and task state]
   F -- Yes --> H{Expected alerts generated?}
   H -- No --> I[Check threshold, detection region, and Alert Output]
   H -- Yes --> J[Business flow is healthy]
```

!!! tip "Troubleshooting order"
   Check **device connection → plugin status → task configuration → runtime node status → output results**. Fix upstream problems before investigating video, alerts, or recordings, and avoid changing several parameters at once.
